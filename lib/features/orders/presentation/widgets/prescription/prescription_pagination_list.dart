...
import 'package:bond/features/orders/presentation/widgets/prescription/prescription_item_card.dart';
...
class PrescriptionPaginationList extends StatelessWidget {
  final PagingController<int, PrescriptionOrderModel> pagingController;
  final Function(PrescriptionOrderModel)? onOrderTap;

  const PrescriptionPaginationList({
    super.key,
  ...
  Widget build(BuildContext context) {
    return SliverPadding(
      ...
      sliver: PagingListener<int, PrescriptionOrderModel>(
        controller: pagingController,
        builder: (context, state, fetchNextPage) => PagedSliverList(
          state: state,
          fetchNextPage: fetchNextPage,
          builderDelegate: PagedChildBuilderDelegate<PrescriptionOrderModel>(
            ...
            itemBuilder: (context, order, index) =>
                PrescriptionItemCard(order: order, index: index),
          ),
        ),
      ),
    );
